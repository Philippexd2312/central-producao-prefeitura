...
import { getOpenAISettings } from '@/lib/integrations';
...
function detectDepartment(text: string) {
  ...
  for (const code of ['SEMSA', 'SEMED', 'SEMMAS', 'SEMTURDE']) if (normalized.includes(code)) return code;
...
export async function readDocument(bytes: Uint8Array, mimeType: string, filename = 'documento.pdf') {
  ...
  const data = await openAIResponse({
    ...
    input: [{ role: 'user', content: [
      { type: 'input_text', text: 'Leia este documento recebido pela comunicação da prefeitura. Extraia o pedido, texto principal, datas, horários, locais, secretaria, nomes e qualquer informação necessária para produzir a peça. Responda em português e de forma objetiva.' },
...
async function aiTriage(text: string): Promise<TriageResult> {
  ...
  const data = await openAIResponse({
    ...
    text: { format: { type: 'json_schema', name: 'municipal_communication_demand', strict: true, schema: {
      type: 'object', additionalProperties: false,
      properties: {
        title: { type: 'string' }, briefing: { type: 'string' }, revisedText: { type: 'string' }, missingInfo: { type: ['string', 'null'] },
        priority: { type: 'string', enum: ['LOW', 'NORMAL', 'HIGH', 'URGENT'] },
        type: { type: 'string', enum: ['DESIGN', 'VIDEO', 'PHOTO', 'COPY', 'SOCIAL', 'OTHER'] },
        departmentCode: { type: ['string', 'null'] }, dueAtIso: { type: ['string', 'null'] },
      },
  ...
  return {
    title: parsed.title, briefing: parsed.briefing, revisedText: parsed.revisedText, missingInfo: parsed.missingInfo,
    priority: parsed.priority as DemandPriority, type: parsed.type as DemandType, departmentCode: parsed.departmentCode, dueAtIso: parsed.dueAtIso,
  };
}
...
function fallbackTriage(text: string): TriageResult {
  ...
  return {
    title: makeTitle(text), briefing: text || 'Demanda recebida sem texto. Verificar os materiais anexados.', revisedText: text,
    missingInfo: missing.length ? `Confirmar: ${missing.join(' e ')}.` : null, priority: detectPriority(text), type: detectType(text),
    departmentCode: detectDepartment(text), dueAtIso: null,
  };
}
...
export async function triageDemand(input: TriageInput): Promise<TriageResult> {
  ...
  if (settings.apiKey && text) {
    try { return await aiTriage(text); } catch (error) { console.error('AI_TRIAGE_FALLBACK', error); }
  }
